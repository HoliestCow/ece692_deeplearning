...
def store_sequence(targetfile, labels):
    ...
    if np.sum(hits) == 1 or np.sum(hits) == 0:
        return np.zeros((0, 1024)), np.zeros((0,))

    tostore_spectra = current_slice
    tostore_labels = np.zeros((tostore_spectra.shape[0], ), dtype=int)
    tostore_labels[hits] = int(string2id[source_type]) - 1

    # THIS IS THE JUST SOURCE PORTION
    desired_index = tostore_labels != 0
    tostore_spectra = tostore_spectra[desired_index, :]
    tostore_labels = tostore_labels[desired_index]

    # grphandle.create_dataset('measured_spectra', data=tostore_spectra, compression='gzip')
    # grphandle.create_dataset('labels', data=tostore_labels, compression='gzip')
    ...
    return tostore_spectra, tostore_labels
...
def main():
    ...
    f = h5py.File('sequential_dataset_relabel_justsources.h5', 'w')
    ...
    for i in range(len(sourcefilelist_train)):
        ...
        spectra, classifications = store_sequence(current_file, labels)
        tostore_spectra += [spectra]
        tostore_labels += [classifications]

    tostore_spectra = np.concatenate(tostore_spectra, axis=0)
    tostore_labels = np.concatenate(tostore_labels, axis=0)

    train.create_dataset('measured_spectra', data=tostore_spectra)
    train.create_dataset('labels', data=tostore_labels)
    ...
    test.create_dataset('measured_spectra', data=tostore_spectra, compression='gzip')
