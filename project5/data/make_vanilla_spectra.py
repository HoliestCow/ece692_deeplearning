...
def main():
    ...
    tostore_spectra = np.zeros((number_of_training_files, 1024))
    tostore_labels = np.zeros((number_of_training_files, 1))
    filelist = list(labels.keys())
    for i in range(number_of_training_files):
        # create training dataset
        random_file = filelist[i]
        if i % 100 == 0:
        ...
        yolo = source
        t = int(labels[random_file]['time'])
        spectra = x[:, 1:]
        source = int(string2id[source])
        tostore_labels = np.zeros((spectra.shape[0],))
        if source != 0:
            tostore_labels[t - 2:t + 3] = np.array([source, source, source, source, source])
        g = train.create_group('sample_' + str(i))
        g.create_dataset('spectra', data=spectra, compression='gzip')
        g.create_dataset('label', data=tostore_labels)
    ...
    tostore_spectra = np.zeros((int(number_of_testing_files), 1024))
    tostore_labels = np.zeros((int(number_of_testing_files), 1))
